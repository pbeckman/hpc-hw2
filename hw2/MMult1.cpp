...
// more like
// g++ -std=c++11 -fopenmp -O3 -march=native -o MMult1 MMult1.cpp && ./MMult1
...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  ...
  for (long jb = 0; jb < n / BLOCK_SIZE; jb++) {
    for (long pb = 0; pb < k / BLOCK_SIZE; pb++) {
      for (long ib = 0; ib < m / BLOCK_SIZE; ib++) {
        for (long j = 0; j < BLOCK_SIZE; j++) {
          for (long p = 0; p < BLOCK_SIZE; p++) {
            for (long i = 0; i < BLOCK_SIZE; i++) {
              double A_ip = a[ib*BLOCK_SIZE+i + (pb*BLOCK_SIZE+p)*m];
              double B_pj = b[pb*BLOCK_SIZE+p + (jb*BLOCK_SIZE+j)*k];
              double C_ij = c[ib*BLOCK_SIZE+i + (jb*BLOCK_SIZE+j)*m];
              C_ij = C_ij + A_ip * B_pj;
              c[ib*BLOCK_SIZE+i + (jb*BLOCK_SIZE+j)*m] = C_ij;
            }
          }
        }
      }
...
int main(int argc, char** argv) {
  ...
  const long PINC = std::max(200/BLOCK_SIZE,1) * BLOCK_SIZE; // multiple of BLOCK_SIZE
